 ...
 *
 * Run: npx convex run functions/resetFY2526Calendar:resetAndImport
 */
export const resetAndImport = mutation({
  args: {},
  returns: v.object({
  ...
  handler: async (ctx) => {
    ...
    for (const fy of stale) {
      const calendars = await ctx.db
        ...
        .withIndex("by_fiscalYear", (q) => q.eq("fiscalYearId", fy._id))
        .collect();
      for (const cal of calendars) {
        const cells = await ctx.db
          .query("assignments")
          .withIndex("by_calendar", (q) => q.eq("masterCalendarId", cal._id))
          .collect();
      ...
      const rotations = await ctx.db
        ...
        .withIndex("by_fiscalYear", (q) => q.eq("fiscalYearId", fy._id))
        .collect();
      for (const r of rotations) await ctx.db.delete(r._id);

      await ctx.db.delete(fy._id);
    }

    // ── Step 2: Create fresh fiscal year ────────────────────────────────────
    ...
    for (const rot of rotationDefs) {
      const id = await ctx.db.insert("rotations", {
        fiscalYearId,
        name: rot.name,
      ...
      rotationIdMap.set(rot.abbreviation, id);
    }

    // ── Step 4: Create weeks (54) ────────────────────────────────────────────
    const weekDefs = [
    ...
    for (const week of weekDefs) {
      ...
      weekIdMap.set(week.weekNumber, id);
    }

    // ── Step 5: Look up existing physicians by initials ──────────────────────
    const allPhysicians = await ctx.db.query("physicians").collect();
    const physicianByInitials = new Map(
      allPhysicians.map((p) => [p.initials.trim().toUpperCase(), p._id]),
    );

    // ── Step 6: Create master calendar (published) ───────────────────────────
    ...
    // ── Step 7: Import assignments from Excel ────────────────────────────────
    // Week 47 Pulm: "SP covering for EP" → extracted as "SP" (matches Excel)
    // Week 54: all blank — no assignments inserted
    const assignmentData: { week: number; rotation: string; physician: string }[] = [
      ...
      // Week 47 Pulm: Excel reads "SP covering for EP" → extracted as SP
      ...
      // Week 54: all blank in Excel — no assignments
    ];

    const unknownInitialsSet = new Set<string>();
    let assignmentsCreated = 0;

    for (const asgn of assignmentData) {
      const weekId = weekIdMap.get(asgn.week);
      const rotationId = rotationIdMap.get(asgn.rotation);
      const physicianId = physicianByInitials.get(asgn.physician.toUpperCase());

      if (!physicianId) {
        unknownInitialsSet.add(asgn.physician);
        continue;
      }
      if (!weekId || !rotationId) continue;
    ...
    return {
      fiscalYearId,
      masterCalendarId,
      weeksCreated: weekDefs.length,
      rotationsCreated: rotationDefs.length,
      assignmentsCreated,
