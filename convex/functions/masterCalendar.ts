...
function sortWeeksByWeekNumber(weeks: Doc<"weeks">[]) {
  return [...weeks].sort((a, b) => a.weekNumber - b.weekNumber);
}

function sortActiveRotations(rotations: Doc<"rotations">[]) {
  return rotations
    .filter((rotation) => rotation.isActive)
    .sort((a, b) => a.sortOrder - b.sortOrder);
}

function sortActivePhysicians(physicians: Doc<"physicians">[]) {
  return physicians
...
async function getDraftCalendarForFiscalYear(
  ...
  return (
    calendars
      .filter((calendar: Doc<"masterCalendars">) => calendar.status === "draft")
      .sort((a: Doc<"masterCalendars">, b: Doc<"masterCalendars">) => b.version - a.version)[0] ?? null
  );
}
...
async function getAvailabilityByPhysicianWeek(
  ...
  for (const request of requestByPhysician.values()) {
    const preferences = await ctx.db
      ...
      .withIndex("by_request", (q: any) => q.eq("scheduleRequestId", request._id))
      .collect();
...
async function getClinicCfteByPhysician(
  ...
  const cftePerHalfDayByClinicTypeId = new Map<string, number>(
    clinicTypes.map((clinicType: Doc<"clinicTypes">) => [
...
async function getTargetCfteByPhysician(
  ...
  return new Map<string, number>(
    targets.map((target: Doc<"physicianCfteTargets">) => [String(target.physicianId), target.targetCfte]),
  );
}
...
async function getCfteSummary({
  ...
  const rotationsById = new Map<string, Doc<"rotations">>(
    rotations.map((rotation: Doc<"rotations">) => [String(rotation._id), rotation]),
  );
  ...
  const assignments: Doc<"assignments">[] =
    calendarId === null
      ...
      : await ctx.db
          .query("assignments")
          .withIndex("by_calendar", (q: any) => q.eq("masterCalendarId", calendarId))
          .collect();
...
export const getCurrentFiscalYearMasterCalendarDraft = query({
  ...
  handler: async (ctx) => {
    ...
    const [weeks, rotations, physicians] = await Promise.all([
      ctx.db
        .query("weeks")
        .withIndex("by_fiscalYear", (q: any) => q.eq("fiscalYearId", fiscalYear._id))
        .collect(),
      ctx.db
        .query("rotations")
        .withIndex("by_fiscalYear", (q: any) => q.eq("fiscalYearId", fiscalYear._id))
    ...
    const assignments = await ctx.db
      .query("assignments")
      .withIndex("by_calendar", (q: any) => q.eq("masterCalendarId", draftCalendar._id))
      .collect();
...
export const assignCurrentFiscalYearDraftCell = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const assignment = await ctx.db
      .query("assignments")
      .withIndex("by_calendar_week_rotation", (q: any) =>
        q
    ...
    await ctx.db.patch(assignment._id, {
      ...
      assignedBy: args.physicianId ? admin._id : undefined,
